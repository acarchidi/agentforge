---
name: agentforge
description: DeFi safety and analysis APIs for wallet safety, token risk / rug check scoring, transaction decoding, smart contract docs, and text AI (sentiment, summarize, translate). Use when an agent needs to check wallet safety, scan token risk, decode a transaction, get smart contract documentation, or run any DeFi safety check before executing an on-chain action. Pay-per-call via x402 (USDC on Base), no API keys or accounts. Free entry points available for discovery before paying.
metadata:
  author: agentforge
  version: "1.4.0"
---
...
AgentForge is a DeFi safety layer: 16 pay-per-call API endpoints an agent can hit before or during an on-chain action. Every endpoint is priced in USDC on Base and paid via the [x402 protocol](https://www.x402.org/) — no API keys, no accounts, no subscriptions. Base URL: `https://agentforge-taupe.vercel.app`.
...
## Free entry points (no payment, use these first)

- `GET /registry/lookup?address=0x...&chain=ethereum` — look up a contract in our 384-contract label registry (protocol name, category, risk level) before paying for a deeper check.
- `GET /catalog` — full machine-readable catalog of all 16 endpoints with current prices and JSON schemas.
- `GET /about` — service overview, all endpoints with descriptions and `input_example` payloads.
- `GET /.well-known/x402` — x402-specific discovery document (accepted networks, assets, payTo address).
- `POST /feedback` — free, no payment; report a bad response or suggest an improvement.
- `GET /v1/ping` ($0.001) — minimal paid call to verify your x402 client is wired up correctly before spending on a real endpoint.

## MCP server (no-payment alternative)

Every paid endpoint is also exposed as an MCP tool at `https://agentforge-taupe.vercel.app/mcp` (Streamable HTTP transport). If your agent runtime supports MCP directly, you can call these tools without handling x402 payment yourself — the server still requires x402 payment per call, but MCP-native clients that already have an x402-aware wallet wired in (e.g. via `@coinbase/payments-mcp` or similar) can call tools directly instead of doing raw HTTP.
